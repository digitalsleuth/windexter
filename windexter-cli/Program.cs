...
namespace windexter_cli
{
    ...
    class Program
    {
        ...
        {
            [Option('f', "file", Required = true, HelpText = "Path to the database.")]
            public string? File { get; set; }

            [Option('o', "output", Required = false, HelpText = "Path to the output directory.")]
            public string? Output { get; set; }

            [Option("all", HelpText = "Filter for all options.")]
            public bool AllFilterOptions { get; set; }

            [Option('a', "activity", HelpText = "Filter for user activity.")]
            public bool Activity { get; set; }

            [Option('c', "compinfo", HelpText = "Filter for Computer Info.")]
            public bool CompInfo { get; set; }

            [Option('g', "gps", HelpText = "Filter for GPS data.")]
            public bool Gps { get; set; }

            [Option('s', "summary", HelpText = "Filter for Search Summary.")]
            public bool Summary { get; set; }

            [Option('t', "timeline", HelpText = "Generate a Timeline for all data.")]
        ...
        static async Task Main(string[] args)
        {
            ...
            result.WithNotParsed(errs =>
            {
                var helpText = HelpText.AutoBuild(result, h =>
                {
                    ...
                    h.Heading = $"{displayName} {appVersion}";
                    h.Copyright = $"Digital Sleuth - {githubBinaryRepo}";
        ...
        static List<string> GetSelectedFlags(Options opts)
        {
            return [.. typeof(Options)
                .GetProperties()
                .Where(p => p.PropertyType == typeof(bool))
                .Where(p => (bool)p.GetValue(opts)! == true)
                .Select(p => p.Name)];
        }
        ...
        static async Task RunOptions(Options opts)
        {
            ...
            bool anySubParserSelected = typeof(Options)
                .GetProperties()
                .Where(p => p.PropertyType == typeof(bool))
                .Any(p => (bool)p.GetValue(opts)!);
            if (File.Exists(opts.File))
            {
                FileInfo dbInfo = new(opts.File);
                dbFile = dbInfo.FullName;
            }
            else
            {
                Console.WriteLine($"{dbFile} does not exist! Please check your path and try again.");
                return;
            }
            if (opts.Output == "" || opts.Output is null)
            {
                FileInfo fi = new(dbFile);
            ...
            {
                Console.WriteLine($"{outputPath} does not exist! Please check your path and try again.");
                return;
            }
            Console.WriteLine($"{displayName} - {appVersion}");
            Console.WriteLine($"Digital Sleuth - {githubBinaryRepo}\n");
            ...
            if (!dbFile.Contains("gather") && dbType == "index")
            {
                ...
                if (GatherAvailable && opts.Gather)
                {
                ...
                if (PropMapAvailable && opts.PropMap)
                {
            ...
            Console.WriteLine("Extraction and parsing completed!");
            nowDt = DateTime.Now.ToUniversalTime();
            now = nowDt.ToString("yyyyMMdd-HHmmss");
            Console.WriteLine($"Output File: {outputFile}");
            Console.WriteLine($"Completed: {now}");
        }
        ...
        public static async Task ParseSubData(Options opts)
        {
            if (dbType == "index" || dbType == "esedb")
            {
                if (opts.Url || opts.AllFilterOptions)
                {
                ...
                if (opts.Gps || opts.AllFilterOptions)
                {
                    var selectedColumns = new List<string>
                    {
                        "WorkId",
                ...
                if (opts.Summary || opts.AllFilterOptions)
                {
                    var selectedColumns = new List<string>
                    {
                        "WorkId",
                ...
                if (opts.CompInfo || opts.AllFilterOptions)
                {
                    var selectedColumns = new List<string>
                    {
                        "WorkId",
                ...
                if (opts.Activity || opts.AllFilterOptions)
                {
                    var selectedColumns = new List<string>
                    {
                        "WorkId",
                ...
                if (opts.Timeline || opts.AllFilterOptions)
                {
        ...
        private static List<List<object>> ReadAndStoreTable(sqlite3 db, string sql)
        {
            ...
            {
                Console.WriteLine($"Unable to Read and Store data:\n\n{ex.Message}");
            }
            finally
            {
                if (stmt != null)
        ...
        private static async Task<bool> ExportToExcel(string filePath, Dictionary<string, List<List<object>>> dataToExport)
        {
            ...
            {
                Console.WriteLine($"An error occurred while exporting:\n\n{ex.Message}");
                return false;
            }
        }
